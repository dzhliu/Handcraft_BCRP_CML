...
def parse_args_attack_activation():
    ...
    parser.add_argument('--topk_ratio_coarse', type=float, default=0.05) # top k for bd neuron (num of neurons)
    parser.add_argument('--topk_ratio_fine', type=float, default=0.05) # top k for bd weights
    parser.add_argument('--alpha', type=float, default=0.5)  #10 work
...
if False:
    ...
    _, idx_fc0 = torch.topk(torch.abs(w_a), math.ceil(args.topk_ratio_fine * 25088), largest=True)  # 34*410*2509
...
util.test_backdoor_model(benign_model, test_loader, args.target_label, args.attack_ratio, args.attack_mode, args.device, strength=5)
